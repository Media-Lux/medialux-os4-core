...
export async function registerRoutes(
  ...
  // ==========================================================
  // POST /api/branch-push – Push files to non-main branches (Agent3 authorized)
  // Server uses OS4_TRESOR_PUSH_KEY internally, target branch MUST NOT be main
  // ==========================================================
  app.post("/api/branch-push", async (req, res) => {
    try {
      const storedKey = process.env.OS4_TRESOR_PUSH_KEY;
      if (!storedKey) {
        return res.status(500).json({
      ...
      const { path, content, commitMessage, branch, sha } = req.body;
      ...
      if (!commitMessage || typeof commitMessage !== "string") {
        return res.status(400).json({ success: false, error: "commitMessage required" });
      }
      if (!branch || typeof branch !== "string") {
        return res.status(400).json({ success: false, error: "branch required" });
      }
      if (branch === "main") {
        return res.status(403).json({
          success: false,
          error: "Cannot push to main branch via branch-push endpoint",
        });
      }
      ...
      // Check if branch exists, create from main if not
      try {
        ...
        if (branchErr.status === 404) {
          ...
          console.log(`[BRANCH-PUSH] Created branch ${branch} from main`);
        } else {
          throw branchErr;
        }
      }
      ...
      const result = await octokit.repos.createOrUpdateFileContents(pushPayload);

      console.log(`[BRANCH-PUSH] ${path} -> ${branch} sha=${result.data.commit.sha?.slice(0, 7)}`);
      res.json({
        success: true,
        path,
        branch,
      ...
      console.error(`[BRANCH-PUSH] error: ${error.message}`);
      if (error.status === 401) clearGitHubCache();
      res.status(error.status || 500).json({
        success: false,
        error: error.message || "Failed to push to branch",
      });
    }
  });
